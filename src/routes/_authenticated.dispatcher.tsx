...
function DispatcherPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="border-b border-primary/40 p-3 flex items-center justify-between">
        <div>
          <h1 className="text-xl text-primary glow-text font-display">▸ DISPEČINK · {callSign}</h1>
          <div className="text-[10px] text-muted-foreground">VINNÉ TAXI · {drivers.filter(d => d.online).length}/{drivers.length} ONLINE</div>
        </div>
        <button onClick={() => supabase.auth.signOut()} className="border border-primary/40 px-3 py-1 text-xs hover:border-primary flex items-center gap-1">
              ...
              <div key={o.id} className="border-b border-primary/20 p-3 text-sm">
                <div className="flex justify-between items-start gap-2">
                  <div className="flex-1 min-w-0">
                    ...
                    {o.customer_name && <div className="text-xs">{o.customer_name} {o.customer_phone && `· ${o.customer_phone}`}</div>}
                  </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/85 z-40 flex items-center justify-center p-4">
      <form onSubmit={submit} className="bg-black border border-primary glow p-5 max-w-md w-full space-y-3">
        ...
        <In label="ADRESA VYZVEDNUTÍ *" value={pickup} onChange={setPickup} required />
        <In label="CÍL" value={destination} onChange={setDestination} />
        <In label="ZÁKAZNÍK" value={name} onChange={setName} />
        <In label="TELEFON" value={phone} onChange={setPhone} />
